  ...
  return (
    <div className="rounded-lg border border-ink-200 bg-white p-5">
      <div className="flex items-baseline justify-between">
        <h3 className="font-semibold text-ink-900">Price breakdown</h3>
          ...
          return (
            <div key={item.key} className="flex items-start justify-between gap-4">
              <dt className="text-ink-600">
                {item.label}
                {item.detail && <span className="block text-xs text-ink-400">{item.detail}</span>}
              </dt>
              <dd className={isDiscount ? 'font-medium text-emerald-700' : 'font-medium text-ink-900'}>
                {isDiscount ? '' : ''}
      ...
      <div className="mt-4 border-t border-ink-200 pt-3">
        <div className="flex items-baseline justify-between">
          <span className="font-semibold text-ink-900">Rental total</span>
        ...
        <div className="mt-3 flex items-baseline justify-between border-t border-ink-200 pt-3">
          <span className="font-semibold text-ink-900">Total payable now</span>
      ...
      <p className="mt-3 text-xs text-ink-400">{period.dayCountingRule}</p>
    </div>
  );
}
